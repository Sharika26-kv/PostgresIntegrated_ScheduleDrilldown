...
DROP VIEW IF EXISTS final_activity_kpi_view CASCADE;
...
-- Create the final_activity_kpi_view
CREATE VIEW final_activity_kpi_view AS
SELECT 
    t.proj_id as project_id,
    t.task_id,
    t.task_code,
    t.task_name,
    -- Calculate relationship percentage based on schedule progress
    CASE 
        WHEN CAST(t.target_drtn_hr_cnt AS REAL) > 0 THEN 
            CAST(t.phys_complete_pct AS REAL) / 100.0
        ELSE 0
    END as relationship_percentage,
    t.phys_complete_pct,
    t.status_code,
    t.target_start_date,
    t.target_end_date,
